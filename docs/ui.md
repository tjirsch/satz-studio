...
- **The estate coroutine** (`src/state/estate_actions.rs`, `EstateAction`) is started by
  ...
  it. `RunTool { name, args }` calls one MCP tool and puts its text and its
  ...
  - `MergePresets` calls `satz_merge_presets`, which reconciles the estate's library
    with upstream, with the outcome in the command log as `RunTool` puts it.
...
- **Diagnostics drawer:** a bottom drawer, collapsed to its header, listing the open
  estate's diagnostics grouped Errors, Warnings, Info, each with `file:line` relative
...
4. **Answer a question.** Decisions → the first open question → Accept (or type a
   ...
   raised for that param is gone from the drawer. The card is still that question, its
   chip now reading "answered: …" and its forward button reading Next, and its row is
   still where it was in the list although "Show answered" is off. Press Next: the card
   moves to the following question and the answered one leaves the list. Press Back: the
   switch goes on and the card returns to the question just answered.

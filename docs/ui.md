...
1. **Import.** Import → an empty folder → Terraform HCL → a directory
   ...
   On "State document" the Organization field is there, and the preview carries
   `--organization <n>` once it holds a value; on "Live scope" the field is gone.

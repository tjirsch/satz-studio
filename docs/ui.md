...
4. **Answer a question.** Decisions → the first open question → Accept (or type a
   ...
   chip now reading "answered: …", its filled button reading Next and no text Skip/Next beside it, and its row is
   still where it was in the list although "Show answered" is off. Type into the field: the filled button reads Answer and the text Next is back; restore the value and it reads Next again. Press Next: the card

...
| [0015](0015-the-packs-view-draws-the-dependency-tree-the-packs-declare.md) | the Packs view draws the dependency tree the packs declare: the edges are the packs' `ask_when`s read with satz-core's parser, a pack others wait on is a block across the grid with its dependents hung below it, and the right-angle connectors are CSS on the tree's own elements | accepted |

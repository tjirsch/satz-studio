...
| [0020](0020-the-agent-is-an-external-client-that-studio-configures-and-starts.md) | satz-studio runs no model: the chat and both its engines are gone, and the Agent destination writes the estate's `satz mcp` configuration for an external client — `.mcp.json` or a Claude Desktop block — and starts that client in the estate's directory | accepted |

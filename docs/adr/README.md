...
| [0020](0020-the-agent-is-an-external-client-that-studio-configures-and-starts.md) | satz-studio runs no model: the chat and both its engines are gone, and the Agent destination configures an external client on the open estate — `.mcp.json` or Claude Desktop's own file — and starts that client in the estate's directory | accepted; amended 2026-09-23, the configuration is satz's and the app calls `satz mcp-config` for it |

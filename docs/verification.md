...
| `satz questions --format json` through `SatzCli::json` reports the same questions and summary as the last `satz_interview` | `tests/e2e_interview.rs` | same |
...
| At `validation_level = "error"` the same sentence is a refusal: `CliChecker` returns it as one error diagnostic, and a commit under that checker is `Rollback::Check` with it, the file's bytes kept, no temp file left | `tests/e2e_map.rs` | same |
...
| `McpChecker` and `CliChecker` return the same verdict and, for a refusal, the same `(line, message)` set, on six edits | `tests/edit_checkers_agree.rs` | `the_two_checkers_agree_on_six_cases` |
| `ReplaceParam` writes the bytes satz's `bind` writes on an unaligned line; on an aligned line they differ only in the whitespace before `=` | `tests/edit_bind_parity.rs` | both tests |
...
| `McpSession` lists the twenty tools of `docs/mcp.md`, reads `runs_as`, types a tool call; `satz_transpile` under `--allow read` is `is_error` | `tests/satz_mcp.rs`, `tests/satz_session.rs` | all |

...
| Every question of every pack under `vendor/satz/presets` is answered in the shape its param is declared with, in the pack's own text, a reference followed: a list is a list of strings, a bool a bool, a number a number — as the questions report has the row, and again with nothing offered. The sweep runs over one estate that uses every pack, through the installed satz's report and the model the app builds; the report carries every question the library declares, and the list params `access_approval_notification_emails`, `allowed_resource_locations`, `admin_port_source_ranges` and `allowed_policy_member_subjects` are among the lists found. A pin bump that brings a pack whose field would not match its declaration fails here | `tests/e2e_answer_shapes.rs` | `every_question_of_every_pack_is_answered_in_the_shape_its_param_is_declared_with` |
| On a skeleton with the access-approval pack in, `access_approval_notification_emails` offers nothing and blocks; the fold carries the pack's `[]`, so its field is a list; one address sent as a JSON array through `satz_interview` is written as a list of one, the check passes, the model's row for it is a `List`, and the question reads answered with that array | `tests/e2e_answer_shapes.rs` | `one_address_for_a_list_param_is_written_as_a_list_of_one` |
| The offered value decides an answer's shape; a question that offers nothing takes its param's shape in the fold, a param the fold does not bind is a string, and with the fold unknown there is no shape; the fold's YAML and the report's JSON have one rule. A list field starts empty, is empty until a value is added, and sends one value as a JSON array of one | `src/model/params.rs`, `crates/satz-studio/src/components/typed_field.rs` | `the_offered_value_decides_and_a_question_that_offers_none_takes_its_declared_shape`, `the_fold_s_shapes_agree_with_the_offered_value_s`, `one_value_in_a_list_field_is_sent_as_a_list_of_one` |

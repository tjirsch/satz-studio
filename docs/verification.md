...
| `McpSession` lists the twenty-one tools of `docs/mcp.md`, reads `runs_as`, types a tool call; `satz_transpile` under `--allow read` is `is_error` | `tests/satz_mcp.rs`, `tests/satz_session.rs` | all |

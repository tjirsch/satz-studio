...
| A refusal at `validation_level = "error"` is the same findings through both checkers — the JSON over MCP and the `Debug` of the refusal on the CLI — one `unadopted-pack` error naming the pack and `satz merge-presets` | `tests/edit_checkers_agree.rs` | `a_refusal_is_the_same_findings_through_both_checkers` |
| At satz's default level the same estate passes, and the warning is in the MCP summary's `findings`; the CLI summary carries none, as with the addresses | `tests/edit_checkers_agree.rs` | `a_check_that_passes_carries_its_warnings_in_the_mcp_summary` |

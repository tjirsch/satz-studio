...
fn fixture(name: &str) -> Vec<u8> {
    std::fs::read(format!("{}/tests/fixtures/sse/{name}", env!("CARGO_MANIFEST_DIR"))).expect("the recording exists")
}
...
fn a_text_turn_assembles_the_same_in_small_chunks_and_whole() {
    ...
    let thinking = ContentBlock::Thinking { thinking: "The estate has one folder.".to_string(), signature: "EqQBCgIYAhIM".to_string() };
    let usage = Usage { input_tokens: 25, output_tokens: 15, cache_creation_input_tokens: Some(0), cache_read_input_tokens: Some(0) };
    assert_eq!(
        events,
        vec![
            StreamEvent::Started { id: "msg_01TextTurnExample".to_string(), model: "claude-opus-5".to_string() },
            StreamEvent::ThinkingDelta("The estate has one folder.".to_string()),
            StreamEvent::BlockStop { index: 0, block: thinking.clone() },
            StreamEvent::TextDelta("Hello".to_string()),
            StreamEvent::TextDelta(", operator.".to_string()),
            StreamEvent::BlockStop { index: 1, block: text("Hello, operator.") },
            StreamEvent::Done { stop_reason: StopReason::EndTurn, stop_details: None, usage },
        ]
    );
    assert_eq!(
        response,
...
fn a_tool_turn_concatenates_the_input_json_deltas_and_parses_at_block_stop() {
    ...
    let tool_use = ContentBlock::ToolUse { id: "toolu_01CheckExample".to_string(), name: "satz_transpile_check".to_string(), input: input.clone() };
    assert_eq!(
        events,
        vec![
            StreamEvent::Started { id: "msg_01ToolTurnExample".to_string(), model: "claude-opus-5".to_string() },
            StreamEvent::TextDelta("Checking the estate.".to_string()),
            StreamEvent::BlockStop { index: 0, block: text("Checking the estate.") },
            StreamEvent::ToolUseStart { index: 1, id: "toolu_01CheckExample".to_string(), name: "satz_transpile_check".to_string() },
            StreamEvent::ToolInputDelta { index: 1, partial_json: "{\"est".to_string() },
            StreamEvent::ToolInputDelta { index: 1, partial_json: "ate\": \"estates/ac".to_string() },
            StreamEvent::ToolInputDelta { index: 1, partial_json: "me.satz\"}".to_string() },
            StreamEvent::BlockStop { index: 1, block: tool_use.clone() },
            StreamEvent::Done {
                stop_reason: StopReason::ToolUse,
                stop_details: None,
                usage: Usage { input_tokens: 412, output_tokens: 61, cache_creation_input_tokens: Some(380), cache_read_input_tokens: Some(0) },
            },
        ]
    );
    assert_eq!(response.stop_reason, StopReason::ToolUse);
    assert_eq!(response.content[1], tool_use);
    let ContentBlock::ToolUse { input: parsed, .. } = &response.content[1] else { panic!("a tool_use block") };
...
fn a_refusal_carries_its_stop_details() {
    ...
    let details = response.stop_details.clone().expect("stop_details on a refusal");
    assert_eq!(details.kind, "refusal");
    assert_eq!(details.category.as_deref(), Some("cyber"));
    assert_eq!(details.explanation.as_deref(), Some("The request asks for a working exploit."));
    assert_eq!(details.recommended_model.as_deref(), Some("claude-opus-4-8"));
    assert!(matches!(events.last(), Some(StreamEvent::Done { stop_reason: StopReason::Refusal, stop_details: Some(d), .. }) if d.category.as_deref() == Some("cyber")));
...
fn a_mid_stream_error_event_fails_the_stream_after_what_arrived() {
    ...
    let Some(ClaudeError::Stream(message)) = failure else { panic!("a stream error, got {failure:?}") };
    assert_eq!(message, "overloaded_error: Overloaded");
    assert_eq!(events, vec![StreamEvent::Started { id: "msg_01ErrorExample".to_string(), model: "claude-opus-5".to_string() }, StreamEvent::TextDelta("Looking".to_string())]);
...
fn a_fallback_block_is_kept_verbatim_and_round_trips() {
    ...
    assert_eq!(events[1], StreamEvent::BlockStop { index: 0, block: ContentBlock::Other(raw.clone()) });
    // replayed verbatim: serialising the block is the JSON the API sent, and reading it back is the same block
    assert_eq!(serde_json::to_value(&response.content[0]).expect("serialises"), raw);
...
fn the_decoder_joins_data_lines_accepts_crlf_and_ignores_comments_and_ids() {
    ...
    assert_eq!(events, vec![SseEvent { event: "ping".to_string(), data: "{\"a\":\n 1}".to_string() }, SseEvent { event: "message".to_string(), data: "solo".to_string() }]);
...
fn a_stream_cut_inside_an_event_is_an_error() {
    let mut decoder = SseDecoder::new();
    assert!(decoder.push(b"event: message_stop\ndata: {\"type\":\"message_stop\"}\n").expect("decodes").is_empty());
    assert!(matches!(decoder.finish(), Err(ClaudeError::Stream(m)) if m.contains("inside an event")));
    let mut decoder = SseDecoder::new();
    decoder.push(b"data: {}\n\ndata: {").expect("decodes");
...
fn a_stream_without_message_stop_is_an_error() {
    ...
    assert!(matches!(run(cut, 64), Err(ClaudeError::Stream(m)) if m.contains("message_stop") || m.contains("inside an event")));
}

#[test]
fn a_delta_the_code_does_not_fold_is_an_error() {
    let mut assembler = Assembler::new();
    let start = SseEvent { event: "content_block_start".to_string(), data: r#"{"type":"content_block_start","index":0,"content_block":{"type":"text","text":""}}"#.to_string() };
    ...
    assert!(matches!(assembler.feed(&delta), Err(ClaudeError::Stream(m)) if m.contains("citations_delta")));
    let out_of_order = SseEvent { event: "content_block_start".to_string(), data: r#"{"type":"content_block_start","index":5,"content_block":{"type":"text","text":""}}"#.to_string() };
    assert!(matches!(assembler.feed(&out_of_order), Err(ClaudeError::Stream(m)) if m.contains("content block 5")));
}

#[test]
fn an_event_the_code_does_not_read_is_an_error() {
    let mut assembler = Assembler::new();
    let event = SseEvent { event: "message_compaction".to_string(), data: r#"{"type":"message_compaction"}"#.to_string() };
    assert!(matches!(assembler.feed(&event), Err(ClaudeError::Stream(m)) if m.contains("message_compaction")));
}

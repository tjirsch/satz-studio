...
//! interview step): the seventeen day-0 answers the matrix pipes into `satz interview`,
...
async fn the_app_path_and_the_cli_path_end_in_the_same_estate() {
    ...
    // day 0: the scaffold's seventeen questions, nine of them without a usable default
    let open = support::questions(&session).await;
    assert_eq!(open.summary.total, 17);
    assert_eq!(open.summary.unanswered, 17);
    ...
    for (i, (subject, value)) in support::TYPED.iter().enumerate() {
        ...
        assert_eq!(report.report.questions.len(), 17 - (i + 1));
        assert!(
            report
                .report
                .questions
    ...
    assert_eq!(
        report.written, 10,
        "the eight defaults and the two derived names"
    );
    assert!(report.report.summary.complete);
    assert_eq!(report.report.summary.answered, 17);
    ...
    assert_eq!(cli.questions.len(), 17);
    assert!(
        cli.questions
            .iter()

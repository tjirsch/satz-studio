...
use satz_studio_core::llm::{CacheControl, ChatProvider, ClaudeClient, ContentBlock, Credential, Effort, Message, Request, Role, StreamEvent, SystemBlock, ToolHost, tool_defs};
...
impl ToolHost for MockHost {
    fn tools(&self) -> Vec<ToolInfo> {
        vec![
            ToolInfo {
                ...
                output_schema: Some(serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "diagnostics": {"type": "array"}}})),
                annotations: ToolAnnotations { read_only: Some(true), destructive: Some(false), idempotent: Some(true), open_world: Some(false) },
            },
            ToolInfo {
    ...
    fn call<'a>(&'a self, _name: &'a str, _args: serde_json::Map<String, serde_json::Value>) -> Pin<Box<dyn Future<Output = Result<ToolOutcome, SatzError>> + Send + 'a>> {
        Box::pin(async { Ok(ToolOutcome { structured: None, text: "not called live".to_string(), is_error: true }) })
    }
}
...
async fn a_live_request_streams_and_the_second_reads_the_cache() {
    ...
    let req = Request {
        ...
        max_tokens: 64_000,
        system: vec![SystemBlock { text: format!("{PREAMBLE}\n\n{}\n\n{}", host.instructions(), host.guide()), cache_control: Some(CacheControl::ephemeral()) }],
        messages: vec![Message { role: Role::User, content: vec![ContentBlock::text("Reply with the single word: ready. Call no tool.")] }],
    ...
    assert!(matches!(first.first(), Some(StreamEvent::Started { .. })), "{first:?}");
    let Some(StreamEvent::Done { usage, .. }) = first.last() else { panic!("no Done: {first:?}") };
    println!("first: {usage:?}");

    let second = run(&client, &req).await;
    let Some(StreamEvent::Done { usage, .. }) = second.last() else { panic!("no Done: {second:?}") };
    println!("second: {usage:?}");
    assert!(usage.cache_read_input_tokens.unwrap_or(0) > 0, "the second identical request reads the cache: {usage:?}");
}

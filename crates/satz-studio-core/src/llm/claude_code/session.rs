...
//! the same decoder the API backend uses. A turn ends on the CLI's `result` line.
...
use crate::llm::{AgentEvent, Approval, StopReason, StreamEvent, Usage};
...
impl Session {
    ...
    pub async fn run_turn(
        ...
        match self.turn(text, &events, &cancel).await {
            ...
            Err(e) => {
                let event = match &e {
                    ClaudeCodeError::Cancelled => AgentEvent::Cancelled,
                    other => {
                        AgentEvent::Failed(ClaudeCodeError::Protocol(other.to_string()).into())
                    }
                };

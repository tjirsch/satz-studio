...
use super::{
    Blocks, emit, function_tools, http_client, post_stream, stop_reason, tool_content, tool_names,
};
...
impl Ollama {
    ...
    pub fn body(&self, req: &Request) -> serde_json::Value {
                    ...
                    let calls: Vec<serde_json::Value> = message
                        ...
                        .filter_map(|b| match b {
                            ContentBlock::ToolUse { name, input, .. } => Some(
                                serde_json::json!({"function": {"name": name, "arguments": input}}),
                            ),
                            _ => None,
                        })
                        .collect();
        ...
        let mut body =
            serde_json::json!({"model": self.model, "stream": true, "messages": messages});
    ...
    async fn run_inner(
        ...
        if !lines.pending().is_empty() {
            return Err(ClaudeError::Stream(
                "the stream ended inside a line".to_string(),
            ));
        }
        Err(ClaudeError::Stream(
            "the stream ended before its `done` line".to_string(),
        ))
    }
}
...
impl Fold {
    fn feed(&mut self, line: &str) -> Result<Vec<StreamEvent>, ClaudeError> {
        let chunk: Chunk = serde_json::from_str(line)
            .map_err(|e| ClaudeError::Stream(format!("a line is not an Ollama chat chunk: {e}")))?;
        ...
        if let Some(message) = chunk.message {
            ...
            for call in message.tool_calls.unwrap_or_default() {
                ...
                let index = self
                    .blocks
                    .open_call(id.clone(), call.function.name.clone());
                let arguments = call.function.arguments.to_string();
                self.blocks
                    .call_mut(self.blocks.calls() - 1)
                    .expect("just opened")
                    .3 = arguments.clone();
        ...
        if chunk.done {
            ...
            out.push(StreamEvent::Done {
                stop_reason: stop_reason(chunk.done_reason.as_deref(), calls),
                stop_details: None,
                usage: self.usage,
            });
        }

...
impl ClaudeError {
    ...
    pub fn is_retryable(&self) -> bool {
        matches!(self, ClaudeError::RateLimited { .. } | ClaudeError::Overloaded | ClaudeError::Server { .. } | ClaudeError::Connection(_))
    }
}
...
pub(crate) fn from_status(status: reqwest::StatusCode, headers: &reqwest::header::HeaderMap, body: &str) -> ClaudeError {
    let message = error_message(body);
    match status.as_u16() {
        400 => ClaudeError::BadRequest { message, request_id: header(headers, "request-id") },
        ...
        429 => ClaudeError::RateLimited { retry_after: header(headers, "retry-after").and_then(|v| v.trim().parse::<u64>().ok()).map(Duration::from_secs) },
